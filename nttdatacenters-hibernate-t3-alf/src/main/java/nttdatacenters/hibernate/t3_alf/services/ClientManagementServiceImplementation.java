...
import nttdatacenters.hibernate.t3_alf.persistence.ClientDaoImplementation;
import nttdatacenters.hibernate.t3_alf.persistence.ClientDaoInterface;
...
public class ClientManagementServiceImplementation implements ClientManagementServiceInterface {
	...
	public List<Client> searchByIdentityCardNumberAndMonthlyPrice(Integer identityCardNumber, Double monthlyPrice) {
		...
		if (identityCardNumber != null && monthlyPrice != null) {

			// Obtención de los contratos por precio mensual y porcentaje de impuestos
			clientsList = clientDao.searchByIdentityCardNumberAndMonthlyPrice(identityCardNumber, monthlyPrice);
