...
import nttdatacenters.hibernate.t3_alf.persistence.ContractDaoImplementation;
import nttdatacenters.hibernate.t3_alf.persistence.ContractDaoInterface;
...
public class ContractManagementServiceImplementation implements ContractManagementServiceInterface {
	...
	public List<Contract> searchByMonthlyPriceAndContractTypeTaxRate(Double monthlyPrice, Integer taxRate) {
		...
		// Verificación del precio mensual y del porcentaje de impuestos
		if (monthlyPrice > 0 && taxRate > 0) {

			// Obtención de los contratos por precio mensual y porcentaje de impuestos
			contractsList = contractDao.searchByMonthlyPriceAndContractTypeTaxRate(monthlyPrice, taxRate);
	...
	public List<Contract> searchByExpirationDateAndFirstName(LocalDate expirationDate, String firstName) {
		...
		// Verificación del precio mensual y del porcentaje de impuestos
		if (expirationDate != null && StringUtils.isNotBlank(firstName)) {

			// Obtención de los contratos por precio mensual y porcentaje de impuestos
			contractsList = contractDao.searchByExpirationDateAndFirstName(expirationDate, firstName);
