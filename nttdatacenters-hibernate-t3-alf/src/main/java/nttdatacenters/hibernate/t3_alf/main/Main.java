package nttdatacenters.hibernate.t3_alf.main;
...
import nttdatacenters.hibernate.t3_alf.persistence.Client;
...
import nttdatacenters.hibernate.t3_alf.services.ClientManagementServiceImplementation;
import nttdatacenters.hibernate.t3_alf.services.ClientManagementServiceInterface;
import nttdatacenters.hibernate.t3_alf.services.ContractManagementServiceImplementation;
import nttdatacenters.hibernate.t3_alf.services.ContractManagementServiceInterface;
import nttdatacenters.hibernate.t3_alf.services.ContractTypeManagementServiceImplementation;
import nttdatacenters.hibernate.t3_alf.services.ContractTypeManagementServiceInterface;
...
public class Main {
	...
	public static void main(String[] args) {
		...
		contract1.setContractType(null);
		...
		contract1.setContractType(contractType1);
		...
		contract2.setContractType(contractType2);
		...
		// Se muestran los contratos con un precio mensual menor que el indicado y con
		// un porcentaje de impuestos mayor o igual que el indicado
		searchByMonthlyPriceAndContractTypeTaxRate(12.99, 9);

		// Se muestran los clientes por el número del documento de identidad y un precio
		// mayor al introducido por parámetros
		searchByIdentityCardNumberAndMonthlyPrice(412231435, 11.99);

		// Se muestran los contratos por la fecha de caducidad y el nombre del cliente
		searchByExpirationDateAndFirstName(LocalDate.of(2024, 10, 24), "María");
	...
	/**
	 * Método que obtiene los clientes por Id
	 * 
	 * @param clientId
	 */
	...
	/**
	 * Método que obtiene nos contratos por precio mensual y porcentaje de impuestos
	 * 
	 * @param monthlyPrice
	 * @param taxRate
	 */
	public static void searchByMonthlyPriceAndContractTypeTaxRate(Double monthlyPrice, Integer taxRate) {

		try {

			List<Contract> contracts = contractService.searchByMonthlyPriceAndContractTypeTaxRate(monthlyPrice,
					taxRate);
			for (final Contract contract : contracts) {

				if (log.isInfoEnabled()) {
					log.info(String.format(
							"| ID: %s | FECHA DE VIGENCIA: %s | FECHA DE CADUCIDAD: %s | PRECIO MENSUAL: %s | TIPO DE CONTRATO: %s | PORCENTAJE DE IMPUESTOS: %s |",
							contract.getContractId(), contract.getEffectiveDate(), contract.getExpirationDate(),
							contract.getMonthlyPrice(), contract.getContractType().getType(),
							contract.getContractType().getTaxRate()));
				}
			}
	...
	/**
	 * Método que obtiene los clientes por el número del documento de identidad y el
	 * precio mensual del contrato
	 * 
	 * @param identityCardNumber
	 * @param monthlyPrice
	 */
	public static void searchByIdentityCardNumberAndMonthlyPrice(Integer identityCardNumber, Double monthlyPrice) {

		try {

			List<Client> clients = clientService.searchByIdentityCardNumberAndMonthlyPrice(identityCardNumber,
					monthlyPrice);
			for (final Client client : clients) {
	...
	/**
	 * Método que muestra los contratos por fecha de caducidad y nombre del cliente
	 * 
	 * @param identityCardNumber
	 * @param monthlyPrice
	 */
	public static void searchByExpirationDateAndFirstName(LocalDate expirationDate, String firstName) {

		try {

			List<Contract> contracts = contractService.searchByExpirationDateAndFirstName(expirationDate, firstName);
			for (final Contract contract : contracts) {

				if (log.isInfoEnabled()) {
					log.info(String.format(
							"| ID: %s | FECHA DE VIGENCIA: %s | FECHA DE CADUCIDAD: %s | PRECIO MENSUAL: %s | NOMBRE DEL CLIENTE: %s |",
							contract.getContractId(), contract.getEffectiveDate(), contract.getExpirationDate(),
							contract.getMonthlyPrice(), contract.getClient().getFirstName()));
				}
			}
